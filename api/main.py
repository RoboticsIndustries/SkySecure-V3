...
# ─── L2/L3 detection + fusion ────────────────────────────────────────────────

# Map fused score -> risk band. L1-only escalation is preserved above; this
# adds the L2/L3 contribution that previously never ran.
_THREAT_TO_RISK = {
    "UNKNOWN":  0,
    "LOW":      0,
    "MEDIUM":   45,
...
def run_l2_l3_detection(aircraft_list: List[dict]) -> None:
    """
    Run L2 (kinematic) and L3 (integrity) over every aircraft and fuse with
    the L1 result already attached by run_l1_cross_validation().

    This closes the gap where `anomaly_detector` was constructed at startup
    and then never invoked — L2/L3 existed as modules but contributed nothing
    to any live verdict. Runs on the full list (pure local computation, no
    network calls, so unlike L1 there is no rate limit to respect).

    Note on L3: OpenSky /states/all carries no NIC/NACp, so L3 self-reports as
    unavailable on this feed and is dropped from the weighted sum rather than
    contributing a phantom score. Populating it requires carrying integrity
    metadata over from the adsb.lol/adsb.fi fetch in L1.
    """
    ...
    for ac in aircraft_list:
        ...
        try:
            assessment = anomaly_detector.assess(
                ...
                lon=ac["lon"],
                alt_baro=ac.get("alt"),
                alt_geo=ac.get("alt_geo"),
                velocity=ac.get("vel"),
                vertical_rate=ac.get("vr"),
                heading=ac.get("hdg"),
                nic=ac.get("nic"),
                nac_p=ac.get("nac_p"),
                l1_result=_L1_CACHE.get(icao, (0, None))[1],
                observed_at=now,
            )
        except Exception as e:
            log.warning(f"L2/L3 assessment failed for {icao}: {e}")
            continue

        ac["fused"] = assessment.to_dict()

        # Risk is the max of whatever L1 already set and the fused view, so
        # this never downgrades an L1 SPOOFED escalation.
        ...
        for key, label in (("l2_kinematic", "L2_KINEMATIC"),
                           ("l3_integrity", "L3_INTEGRITY")):
            layer = assessment.layers.get(key)
            if layer and layer.available and layer.score >= anomaly_detector.ASSERT_THRESHOLD:
                ac.setdefault("anoms", []).append({
                    "type": label,
...
async def _fetch_live_aircraft() -> List[dict]:
    ...
    try:
        ...
        # L2/L3 run over the whole batch and fuse with whatever L1 attached.
        run_l2_l3_detection(aircraft)

        log.info(
            f"OpenSky: {len(aircraft)} aircraft "
            f"(L1: {'enabled' if TDOA_AVAILABLE else 'disabled'}, "
            f"L2/L3: {'enabled' if anomaly_detector else 'disabled'})"
        )
        return aircraft
...
async def healthz():
    return {
        ...
        # Per-layer status, reported honestly. L3 is "degraded" rather than
        # enabled because the OpenSky feed carries no NIC/NACp, so the layer
        # self-reports unavailable per aircraft and is excluded from fusion.
        "layers": {
            "l1_cross_source": "active" if cross_validator else "disabled",
            "l2_kinematic": "active" if anomaly_detector else "disabled",
            "l3_integrity": "degraded_no_metadata" if anomaly_detector else "disabled",
            "l4_rf_fingerprint": "not_implemented",
            "l5_galileo_osnma": "not_implemented",
        },
    }
    ...
    try:
        async for msg in consumer:
            ...
            try:
                ...
                # Same L2/L3 fusion as the live path, so an alert delivered
                # over Kafka carries the same multi-layer verdict as one from
                # the polling path rather than an L1-only view.
                run_l2_l3_detection([ac])
